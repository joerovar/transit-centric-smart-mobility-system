...
class NBNorm(nn.Module):
    ...
    def forward(self,x):
        x = x.permute(0,2,1,3)
        #print('Xshape in NB',x.shape,'with cin',self.c_in,' cout',self.c_out)
        ...
        p = F.sigmoid(p)
        #print('n_out shape',n.shape)
        #print(np.mean(n.detach().cpu().numpy()),np.mean(p.detach().cpu().numpy()))
...
class B_TCN(nn.Module):
    ...
    def forward(self, X):
        ...
        # print(Xf.is_cuda,inv_idx.is_cuda)
        ...
        # (batch_size, rec_timesteps, out_features)
  ...
  wx_t  + wx_s
   ...
   x_m     x_m
    ...
    def forward(self, X, A_q, A_h):
        ...
        X_T = X.permute(0,2,1)
        #print('X_S,X_T shape', X.shape,X_T.shape)
        X_t1 = self.TC1(X_T)
        ...
        # Maybe not suitable to directly use plus here!
        #print('ntnbshape',n_t_nb.shape,'nsnbshape',n_s_nb.shape)
        #print(n_s_nb.detach().cpu().numpy().shape,np.mean(n_s_nb.detach().cpu().numpy()))
#        print('outputn',np.nanmean(n_s_nb.detach().cpu().numpy()))
