...
from input import STOPS, CONTROL_STRENGTH_PARAMETER, IDX_RT_PROGRESS, IDX_PAX_AT_STOP
import seaborn as sns
from post_process import save, load, plot_sensitivity_whisker
import matplotlib.pyplot as plt
from datetime import datetime
from output import PostProcessor, policy
st = time.time()
...
def run_base_control_detailed(replications=2, save_results=False, time_dep_tt=True, time_dep_dem=True,
    ...
    if save_results:
        ...
        params = {'param': ['control_strength'],
                  'value': [CONTROL_STRENGTH_PARAMETER]}
...
N_REPLICATIONS = 40
# run_base_control_detailed(replications=2, hold_adj_factor=0.6)
# RUN BENCHMARK
...
# run_base_control_detailed(replications=N_REPLICATIONS, save_results=True, hold_adj_factor=0.75)
# run_base_control_detailed(replications=N_REPLICATIONS, save_results=True, hold_adj_factor=0.5)
...
# t_out = load('out/compare/benchmark/trip_t_sim.pkl')
# fig, ax = plt.subplots()
# axt = ax.twinx()
...
# fig.legend()
# plt.savefig('out/compare/validation/trip_t_dist.png')
...
# results_w = {}
# results_w.update(prc_w.pax_times_fast(include_rbt=False))
...
# results = {}
# results.update(prc.pax_times_fast(include_rbt=True))
#
